...
fn test_gif(path: &Path) -> Option<PaletteTestResult> {
    ...
    let frame_inputs: Vec<FrameInput> = frames.iter().map(|f| {
        FrameInput::new(width, height, f.delay, f.pixels.clone())
    }).collect();
...
fn compare_palette_modes() {
    ...
    for r in &results {
        let winner = if r.shared_size <= r.perframe_size { "<" } else { ">" };
        
        println!("{:<40} {:>4}x{:<4} {:>6} {:>8}KB {:>8}KB {:>8}KB {:>7} {:>7} {}",
    ...
    for r in &results {
        ...
        println!("{:>6} px ({:>4}x{:<4}) {:>6} frames: {} by {:>4}KB ({:.1}%)",
            pixels, r.width, r.height, r.frame_count,
            if shared_better { "SHARED  " } else { "PERFRAME" },
            diff_kb, diff_pct);
    }
    
    println!("\nSummary: shared wins {}, perframe wins {}", shared_wins, perframe_wins);
}
