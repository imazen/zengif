...
fn test_gif_quality(path: &Path) -> Option<QualityResult> {
    ...
    let frame_inputs: Vec<FrameInput> = original_frames.iter().enumerate().map(|(i, pixels)| {
    ...
    let mut output_shared = Vec::new();
    {
        let config = EncoderConfig::new(width, height)
            ...
            .shared_palette(true);
        let mut encoder = Encoder::new(&mut output_shared, config, Limits::none(), Unstoppable).ok()?;
    ...
    let mut output_perframe = Vec::new();
    {
        ...
        let mut encoder = Encoder::new(&mut output_perframe, config, Limits::none(), Unstoppable).ok()?;
        for frame in &frame_inputs {
            encoder.add_frame(frame.clone()).ok()?;
        }
        encoder.finish().ok()?;
    ...
    let n = frame_count.min(shared_frames.len()).min(perframe_frames.len());
    for i in 0..n {
    ...
    let perframe_worst = perframe_scores.iter().cloned().fold(f64::INFINITY, f64::min);
...
fn compare_palette_quality() {
    ...
    for r in &results {
        let size_win = if r.shared_size <= r.perframe_size { "S" } else { "P" };
        let qual_win = if r.shared_ssim2_avg >= r.perframe_ssim2_avg - 0.5 { "S" } else { "P" };
        
        println!("{:<32} {:>4}x{:<4} {:>5} {:>6}KB {:>6}KB {:>8.1} {:>8.1}  {}{}",
    ...
    // Summary
    println!("\n=== Summary ===");
    let shared_smaller = results.iter().filter(|r| r.shared_size <= r.perframe_size).count();
    let shared_better_q = results.iter().filter(|r| r.shared_ssim2_avg >= r.perframe_ssim2_avg - 0.5).count();
    println!("Shared wins SIZE: {}/{}", shared_smaller, results.len());
    println!("Shared wins/ties QUALITY (within 0.5): {}/{}", shared_better_q, results.len());
}
