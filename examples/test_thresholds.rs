...
fn test_gif(path: &Path, thresholds: &[f32]) {
    ...
    if orig_frames.is_empty() { return; }
    
    let inputs: Vec<_> = orig_frames.iter()
        .map(|p| FrameInput::new(w, h, 10, p.clone()))
        .collect();
    
    print!("{:<40} {:>4}x{:<4} {:>3}f  ", &name[..name.len().min(40)], w, h, orig_frames.len());
    ...
    for &threshold in thresholds {
        ...
        let worst_ssim: f64 = orig_frames.iter().zip(enc_frames.iter())
...
fn main() {
    ...
    let problem_gifs = ["glitch", "vaporwave", "newtons", "ocean", "rainbow", "space", "city"];
    
    let mut entries: Vec<_> = fs::read_dir(test_dir).unwrap()
        .filter_map(|e| e.ok())
        .filter(|e| e.path().extension().map(|x| x == "gif").unwrap_or(false))
        .filter(|e| problem_gifs.iter().any(|p| e.path().to_string_lossy().contains(p)))
        .collect();
