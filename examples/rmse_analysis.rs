...
fn compute_frame_rmse(orig: &[Rgba], enc: &[Rgba]) -> f64 {
    if orig.len() != enc.len() || orig.is_empty() { return 0.0; }
    let mut total = 0u64;
    let mut count = 0u64;
    for (o, e) in orig.iter().zip(enc.iter()) {
        if o.a == 0 { continue; }  // Skip transparent pixels
...
fn main() {
    ...
    println!("{:<42} {:>3}f  {:>5} {:>5} {:>5} {:>5}  >15 >10  >5  >2", 
        "GIF", "", "max", "avg", "p90", "p50");
    println!("{}", "-".repeat(90));
    ...
    for entry in &entries {
        ...
        let mut output = Vec::new();
        {
            ...
            for inp in &inputs { enc.add_frame(inp.clone()).unwrap(); }
            enc.finish().unwrap();
        }
        ...
        println!("{:<42} {:>3}f  {:>5.1} {:>5.1} {:>5.1} {:>5.1}  {:>3} {:>3} {:>3} {:>3}",
            &name[..name.len().min(42)], orig_frames.len(), max_rmse, avg_rmse, p90, p50,
            exceed_15, exceed_10, exceed_5, exceed_2);
    }
}
