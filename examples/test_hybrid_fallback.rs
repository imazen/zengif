...
static LOCAL_PALETTE_COUNT: AtomicUsize = AtomicUsize::new(0);
...
fn main() {
    ...
    println!("{:<45} {:>10} {:>6} {:>8} {:>8} {:>8} {:>6} {:>6}",
        "File", "Dims", "Frms", "Sh KB", "Hyb KB", "PF KB", "ShSSIM", "HySSIM");
    println!("{}", "-".repeat(115));
    
    let mut entries: Vec<_> = fs::read_dir(test_dir).unwrap()
        .filter_map(|e| e.ok())
    ...
    for entry in entries {
        ...
        if orig_frames.is_empty() { continue; }
        ...
        let mut out_shared = Vec::new();
        {
            ...
            let mut enc = Encoder::new(&mut out_shared, config, Limits::none(), Unstoppable).unwrap();
            for frame in &inputs { enc.add_frame(frame.clone()).unwrap(); }
            enc.finish().unwrap();
        }
        ...
        let mut out_hybrid = Vec::new();
        {
            ...
            let mut enc = Encoder::new(&mut out_hybrid, config, Limits::none(), Unstoppable).unwrap();
            for frame in &inputs { enc.add_frame(frame.clone()).unwrap(); }
            enc.finish().unwrap();
        }
        ...
        let mut out_perframe = Vec::new();
        {
            ...
            let mut enc = Encoder::new(&mut out_perframe, config, Limits::none(), Unstoppable).unwrap();
            for frame in &inputs { enc.add_frame(frame.clone()).unwrap(); }
            enc.finish().unwrap();
        }
        ...
        let sh_ssim: f64 = if !sh_frames.is_empty() {
            let scores: Vec<f64> = orig_frames.iter().zip(sh_frames.iter())
        ...
        let hy_ssim: f64 = if !hy_frames.is_empty() {
            let scores: Vec<f64> = orig_frames.iter().zip(hy_frames.iter())
                .map(|(o, e)| compute_ssim2(o, e, ww, hh))
                .filter(|&s| s >= 0.0)
                .collect();
            scores.iter().sum::<f64>() / scores.len() as f64
        ...
        let fallback = if out_hybrid.len() != out_shared.len() { "*" } else { "" };
        
        println!("{:<45} {:>4}x{:<4} {:>6} {:>7}KB {:>7}KB {:>7}KB {:>6.1} {:>6.1} {}",
